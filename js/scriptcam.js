...
;(function($) {
	$.fn.scriptcam=function(options) {
		...
		return this.each(function() {
			...
			if (fileExists(decodeURIComponent(data.path)+'swf/scriptcam.swf')) {
				swfobject.embedSWF(decodeURIComponent(data.path)+'swf/scriptcam.swf', opts.id, newWidth, newHeight, '11.6', false, opts, params);
			}
			else {
				alert(decodeURIComponent(data.path)+'swf/scriptcam.swf not found, please check the path parameter');
			}
		});
	};
